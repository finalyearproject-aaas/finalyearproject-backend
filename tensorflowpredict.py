...
efficient.load_weights('C:\group14\datasets\sync-folder/efficientnet-b5_imagenet_1000_notop.h5')
...
def binary_fn(inputs):
        print(" In custom layer function", inputs)

        bool_mask = tf.greater(inputs, 0.5)
        int_mask = tf.cast(bool_mask, dtype=tf.float32)

        integer= tf.reduce_sum(int_mask,axis=1)-1

        bi=tf.where(integer>0,1,0)

        fl=tf.cast(bi, dtype=tf.float32)
        return fl
        # return tf.cast(out,dtype=tf.float32)

model.load_weights('C:\group14\datasets\sync-folder/tensorflow11 weights/model.h5')


bin_model=tf.keras.models.clone_model(model)
bin_model.load_weights('C:\group14\datasets\sync-folder/tensorflow11 weights/model.h5')
bin_model.add(layers.Lambda(binary_fn, name="lambda_layer"))
bin_model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])


im=plt.imread("C:\group14\datasets\Med clg\\0\\1.jpeg"	)
im2=preprocess_image(im)


print(bin_model.predict(im2[np.newaxis]/255)>.5)
...
def predict(image):
    ...
    # res= bin_model.predict(image[np.newaxis]/255)
    res=(bin_model.predict(image[np.newaxis]/255)>.5)
